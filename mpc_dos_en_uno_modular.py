...
def build_model():
    ...
    model.setup()
    return model, h, hT, t_on, t_on_acum, u, R, Qdem, Ce

# =========================
# Construcción del MPC
# =========================
def build_mpc(model, h, hT, t_on, t_on_acum, u, R, Qdem, Ce):
...
def setup_graphics(mpc, simulator):
    ...
    for g in [sim_graphics, mpc_graphics]:
        # Nivel de pozos
        g.add_line(var_type='_x', var_name='h_0', axis=ax1[0])
        g.add_line(var_type='_x', var_name='h_1', axis=ax1[0])
        g.add_line(var_type='_x', var_name='h_2', axis=ax1[0])
...
def run_closed_loop(mpc, simulator, x0, sim_steps=10):
...
# Main
# =========================
def main(sim_steps=20, export_filename='mpc_dos_en_uno_results.csv'):
    model, h, hT, t_on, t_on_acum, u, R, Qdem, Ce = build_model()
    mpc = build_mpc(model, h, hT, t_on, t_on_acum, u, R, Qdem, Ce)
